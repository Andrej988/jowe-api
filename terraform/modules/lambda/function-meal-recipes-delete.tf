data "archive_file" "meal_recipes_delete_zip" {
  type        = "zip"
  source_dir  = "${local.lambda_directories["functions_meal_recipes"]}/delete/"
  output_path = "./temp/meal_recipes_delete.zip"
}

resource "aws_cloudwatch_log_group" "meal_recipes_delete_lambda_log_group" {
  name = "${local.cloudwatch_lambdas_log_group_prefix}${local.lambda_function_names["meal_recipes_delete"]}"
...
resource "aws_lambda_function" "meal_recipes_delete_lambda" {
  filename      = data.archive_file.meal_recipes_delete_zip.output_path
  function_name = local.lambda_function_names["meal_recipes_delete"]
  role          = aws_iam_role.jowe_api_lambda_crud.arn
  handler       = "index.handler"

  source_code_hash = data.archive_file.meal_recipes_delete_zip.output_base64sha256
  ...
  tags = {
    Name        = "${var.APP_NAME}-api-meal-recipes-delete"
    Environment = var.ENV
    App         = var.APP_NAME
  }
