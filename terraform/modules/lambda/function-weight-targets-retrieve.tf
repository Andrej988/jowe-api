data "archive_file" "weight_targets_retrieve_zip" {
  type        = "zip"
  source_dir  = "${local.lambda_directories["functions_weight_targets"]}/retrieve/"
  output_path = "./temp/weight_targets_retrieve.zip"
}

resource "aws_cloudwatch_log_group" "weight_targets_retrieve_lambda_log_group" {
  name = "${local.cloudwatch_lambdas_log_group_prefix}${local.lambda_function_names["weight_targets_retrieve"]}"
...
resource "aws_lambda_function" "weight_targets_retrieve_lambda" {
  ...
  function_name = local.lambda_function_names["weight_targets_retrieve"]
