data "archive_file" "weight_measurements_insert_zip" {
  type        = "zip"
  source_dir  = "${local.lambda_directories["functions_weight_measurements"]}/insert/"
...
resource "aws_cloudwatch_log_group" "weight_measurements_insert_lambda_log_group" {
  name = "${local.cloudwatch_lambdas_log_group_prefix}${local.lambda_function_names["weight_measurements_insert"]}"
...
resource "aws_lambda_function" "weight_measurements_insert_lambda" {
  ...
  function_name = local.lambda_function_names["weight_measurements_insert"]
