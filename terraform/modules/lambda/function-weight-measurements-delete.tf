data "archive_file" "weight_measurements_delete_zip" {
  type        = "zip"
  source_dir  = "${local.lambda_directories["functions_weight_measurements"]}/delete/"
  output_path = "./temp/weight_measurements_delete.zip"
}

resource "aws_cloudwatch_log_group" "weight_measurements_delete_lambda_log_group" {
  name = "${local.cloudwatch_lambdas_log_group_prefix}${local.lambda_function_names["weight_measurements_delete"]}"
...
resource "aws_lambda_function" "weight_measurements_delete_lambda" {
  ...
  function_name = local.lambda_function_names["weight_measurements_delete"]
