data "archive_file" "weight_targets_delete_user_data_zip" {
  type        = "zip"
  source_dir  = "${local.weight_targets_lambdas_directory}/deleteUserData/"
...
resource "aws_cloudwatch_log_group" "weight_targets_delete_user_data_lambda_log_group" {
  name = "${local.cloudwatch_lambdas_log_group_prefix}${local.weight_targets_delete_user_data_lambda_name}"
...
resource "aws_lambda_function" "weight_targets_delete_user_data_lambda" {
  ...
  function_name = local.weight_targets_delete_user_data_lambda_name
