data "archive_file" "weight_measurements_retrieve_zip" {
  type        = "zip"
  source_dir  = "${local.lambda_directories["functions_weight_measurements"]}/retrieve/"
  output_path = "./temp/weight_measurements_retrieve.zip"
}

resource "aws_cloudwatch_log_group" "weight_measurements_retrieve_lambda_log_group" {
  name = "${local.cloudwatch_lambdas_log_group_prefix}${local.lambda_function_names["weight_measurements_retrieve"]}"
...
resource "aws_lambda_function" "weight_measurements_retrieve_lambda" {
  ...
  function_name = local.lambda_function_names["weight_measurements_retrieve"]
  ...
  depends_on = [
    ...
    aws_iam_role_policy_attachment.jowe_api_lambda_dynamodb_read_only_role_attachment_weight_measurements,
